...
function loadDataset(filePath: string, maxCases: number): TestCase[] {
  ...
  for (let i = 0; i < Math.min(lines.length, maxCases); i++) {
    try {
      ...
      // Parse assistant content: { "units": [{ "unit_id": "...", "type": "..." }] }
      const asstData = JSON.parse(asstMsg.content);
      const groundTruth: Record<string, string> = {};
      for (const u of asstData.units ?? []) {
        groundTruth[u.unit_id ?? u.unitId] = u.type;
      }
...
async function main() {
  ...
  for (let i = 0; i < cases.length; i++) {
    ...
    try {
      ...
      // Build prediction map: unit_id → type
      const predictions: Record<string, string> = {};
      for (const u of result.data.units) {
        predictions[u.unitId] = u.type;
      }
